    ...
    <ScrollView onScroll={(e)=>{
      ...
      <View style={[mainStyle.pdH20,mainStyle.flexBox]}>
        <Text style={{
          ...
          marginTop:20,
          marginBottom:14
        }}>{headlines._latestExpenses}</Text>
        <Button onPress={()=>navigation?.navigate("AllExpenses")} style={{
          borderWidth:0
        }} Icon={()=><SvgXml xml={right}></SvgXml>} title={headlines._more}/>
      </View>
      <ExpensesCart isDark={isDark} textColor={textColor} borderColor={borderColor}/>
