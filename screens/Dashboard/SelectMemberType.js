...
export default function SelectMemberType({ navigation, route }) {
  ...
  return (
    <View style={{flex:1}}>
      <BackHeader title={prevRoute?.name == "SubscriptionDetails"?values.getValues()._selectProvider:isBn?"সদস্য যোগ করুন":"Add member"} navigation={navigation} />
      <ScrollView
        style={{
        ...
        <View style={[{ flex: 1 }, mainStyle.pdH20]}>
          ...
          <Button
            ...
            title={
              prevRoute?.name == "SubscriptionDetails"
                ? headlines._createMemberOwn
                : isBn
                ? "একজন সদস্য তৈরি করুন"
                : "Create a member"
            }
            active={true}
          />
