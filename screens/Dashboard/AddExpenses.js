...
import { Platform, ScrollView, KeyboardAvoidingView, Pressable, Text, Modal, Dimensions } from "react-native";
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ScrollView style={mainStyle.pdH20} showsVerticalScrollIndicator={false}>
        <Input outSideStyle={mainStyle.mt24} level={"Name of expense *"} placeholder={"example: salary, bill, maintaining "}/>
        <Input outSideStyle={mainStyle.mt12} keyboardType={"numeric"} placeholder={"0.00"} level={"amount *"}/>
        <Input  leftIcon={<SvgXml xml={calender}/>} editable={false} onPress={showDatePicker}  outSideStyle={mainStyle.mt12} keyboardType={"numeric"} placeholder={"yyyy/mm/dd"} level={"Date *"}/>
       
        
      </ScrollView>
      <Modal transparent={true} visible={isDatePickerVisible} onRequestClose={hideDatePicker}>
      <DateTimePickerModal 
          isVisible={true}
          mode={'date'}
